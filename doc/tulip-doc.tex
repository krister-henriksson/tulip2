...
inspected, and the largest values are chosen.

For the calculation of \verb+displmax+ the initial and final Cartesian coordinates
are converted into box coordinates (BC), i.e. the coordinate system that is read in
from the compound information file. This coordinate system can be skewed, i.e. non-Cartesian.
From these initial and final box coordinates it is straightforward to divide away the
box lengths to get size-independent box coordinates (SIBC).
These SIBC are then subtracted from each other, and the resulting coordinates are
converted back to Cartesian coordinates. This yields a Cartesian displacement vector
and hence a Cartesian distance for each atom in the MDS cell. The largest distance
is chosen for the value of \verb+displmax+.
