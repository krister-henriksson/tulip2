...
frc_use       = ...  boolean 
# => use atomic forces
frc_use_w     = ...  use weights for force components (default)
# frc_use_u   = ...  use weights for force components
#
...
S1  B11  B12  B13            Species and vector components of first basis atom.
S2  B21  B22  B23            Species and vector components of second basis atom.
...
\end{Verbatim}
...
If the \verb+format+ keyword is \verb+scaled+ or \verb+Scaled+ or letter \verb+s+ or \verb+S+
then \eg the first basis vector is

$\qquad$ $\mathbf{b}_1 =$
\verb+(L * B11)+ $\mathbf{e}_x +$ \verb+(L * B12)+ $\mathbf{e}_y$ \verb+(L * B13)+ $\mathbf{e}_z$

This is equivalent to \textbf{Cartesian} in VASP. If instead \verb+internal+
is used, then the same basis vector is

$\qquad$ $\mathbf{b}_1 = $
\verb+(L * B11)+ $\mathbf{u}_1 + $ \verb+(L * B12)+ $\mathbf{u}_2 + $ \verb+(L * B13)+ $\mathbf{u}_3$.
...
Fe  0.2000000000   0.0000000000   0.0000000000
\end{Verbatim}
