...
(lattice parameters, cohesive energy, mixing energy, formation energy, bulk modulus,
...
Eform         = ...
# Use either Emix or Eform for a given compound, not both!
...
Formation energies can either be used as-is or as ''mixing energies''.
The mixing energy is simply the formation energy normalized with the number of atoms
...
The mixing energy form automatically accounts for any compound duplication
(periodic copies of the original) that might occur if the cell is too small
to be relaxed as is.

If the regular formation energy is used the code keeps track of any
duplication of the original compound in order to present the formation
energy with respect to the number of atoms in the original compound.
...
The first two deprecated properties \verb+Fmax+ and \verb+Pmax+ are calculated in the
last MDS step. The absolute values of all force and pressure components of all atoms are
inspected, and the largest values are chosen. For the calculation of \verb+displmax+
the initial and final positions are compared, for all atoms, and the largest value
selected. Things are complicated due to the possibility that the box size can change:
if an atom does not move relative to its neighbors, i.e. it does not get displaced,
any change in the box size will give a nonzero value for the maximum displacement.

The current implementation for \verb+displmax+ calculates internal positions from
the Cartesian versions at start and end, taking the boundary conditions into consideration,
then subtracts these from each other, and finally calculates the Cartesian version.
Different box sizes at start and at end still affect the calculation.
