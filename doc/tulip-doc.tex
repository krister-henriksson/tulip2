...
\begin{equation}
B_{ij} = \left( 1 + \chi_{ij} \right)^{-{1 \over 2}}
\end{equation}
...
If the Brenner form is used then
...
and $\omega_{ijk}$ is \textbf{not} a separate parameter. Also,
...
# frc_use_u   = ***  boolean, use weights for force components
#
# Format of forces file: Lines of
#   fx  fy  fz  dfx  dfy  dfz
# in eV/fs, where dfx, dfy, dfz is interpreted as weights/uncertainties,
...
Note 1: The space group is determined by a call to the \verb+spglib+ library.
...
Note 2: If a boolean value of logical true is to be input, use
\verb+yes+, \verb+Yes+, \verb+true+, \verb+True+ or the first letter in these strings.
Any other string evaluates to logical false.

\stars

Note 3: If neither weight nor uncertainty is specified for any property a default weight of 1
...
S1  B11  B12  B13 constr1    Species and vector components of first basis atom.
S2  B21  B22  B23 constr2    Species and vector components of second basis atom.
...
\end{Verbatim}

The real primitive vectors are $\mathbf{v}_i = L \mathbf{u}_i$.
...
$\qquad$ $\mathbf{b}_j =$
\verb+(L * Bj1)+ $\mathbf{e}_x +$ \verb+(L * Bj2)+ $\mathbf{e}_y$ \verb+(L * Bj3)+ $\mathbf{e}_z$
...
$\qquad$ $\mathbf{b}_j = $
\verb+(L * Bj1)+ $\mathbf{u}_1 + $ \verb+(L * Bj2)+ $\mathbf{u}_2 + $ \verb+(L * Bj3)+ $\mathbf{u}_3$.
...
A simulation box with box lenghts $L_1, L_2, L_3$ is constructed from
a compound with primitive vectors $\mathbf{v}_1$, $\mathbf{v}_2$, $\mathbf{v}_3$,
...
\begin{eqnarray}
\widetilde{b/a} &=& (\widetilde{L}_2/L_2) / (\widetilde{L}_1/L_1 ) \times b/a \\
\widetilde{c/a} &=& (\widetilde{L}_3/L_3) / (\widetilde{L}_1/L_1 ) \times c/a
\end{eqnarray}

Cohesive energy:

\begin{eqnarray}
\widetilde{E}_{\mathrm{coh}} &=& \widetilde{E}_P/N \\
\widetilde{\Delta E}_{\mathrm{coh}} 
&=& \widetilde{E}_{\mathrm{coh}} - \widetilde{E}_{\mathrm{coh}}^{\mathrm{ref}}
\end{eqnarray}

Here $N$ is the number of atoms in the compound, and
$\widetilde{E}_{\mathrm{coh}}^{\mathrm{ref}}$ is the cohesive energy of the reference
compound, if used.
...
\begin{eqnarray}
\widetilde{E}_{\mathrm{mix}} &=& \widetilde{E}_f/N \\
\widetilde{E}_f &=& \widetilde{E}_p - \sum_s N(s) E_{\mathrm{coh}}(s)
\end{eqnarray}

Here $N(s)$ is the number of atoms of species $s$ in the compound, and
$E_{\mathrm{coh}}(s)$ is the cohesive energy of the ground state of species $s$.
Parametrization for species $s$ must be given.
For instance, if $s$ is C, then ground state might be GRA (graphite).



Bulk modulus $B$ and its pressure derivative $B'(P)$ is calculated from
...
Elastic constants $C_{ij}$ are calculated using volume-non-conserving strains
...
Here $\widetilde{Y}_i$ is the predicted value of property $Y_i$, e.g.
the bulk modulus of the second compound (if two or more compounds are used
and this compound has a bulk modulus specified as a fitting target).
The $t_i$ is either a weight, $t_i = w_i$, if a weight is used for property $Y_i$,
and $t_i = 1/u_i$ if an uncertainty is used.
\textbf{Default: Using weights, with value 1.0. Specifying weight or uncertainty
...
The $s_i = 1$ if scales are not used (\verb+use_data_scales = false+ in the
specifications file), and $s_i = Y_i$ if scales are used.
By using scaled properties all are on an equal footing.
\textbf{Default: Scales are used, specify 'false' to turn it off. If a property
value is positive and less than machine accuracy then it is replaced by the
machine accuracy.}


Assume the predicted value is $\widetilde{Y}_i = Y_i + \delta Y_i$, then
$(\widetilde{Y}_i - Y_i)/s_i = \delta Y_i / Y_i$ if scales are used, and
$(\widetilde{Y}_i - Y_i)/s_i = \delta Y_i$ otherwise.
Suppose the relative change $\delta Y_i/Y_i = 50\%$ for two properties,
but the properties themselves have values of $1$ and $10^{-7}$.
The former choice (using scales) gives balanced contributions to the
merit function, but the latter does not. In worst case scenario
properties with small values will have a small impact on the fitting
process.
...
Consider now e.g. \verb+Fmax+$ = 10^{-10}$.
Let the predicted value be e.g. $10^{-7}$. The term $f_i$ (with $t_i=1$
and using scaling, $s_i=Y_i$) is

$${10^{-7} - 10^{-10} \over 10^{-10}} \sim 10^3,$$
...
Lesson: Do not use \verb+Fmax+, \verb+Pmax+ or \verb+displmax+. Rely on the
